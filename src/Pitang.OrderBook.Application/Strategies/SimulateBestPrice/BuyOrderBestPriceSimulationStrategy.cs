...
public class BuyOrderBestPriceSimulationStrategy : IOrderSimulationStrategy
{
    private const string OPERATION = "buy";
    ...
    {
        var orders = orderBook.Asks.OrderBy(order => double.Parse(order.Price)).ToList();
        return CalculateOrders(orders, quantity);
    }

    private List<OrderDetail> CalculateOrders(IList<OrderBookDetail> orders, double quantity)
    {
